// skullCharacter.js - V-spread bones + 6-bone dash nova (FULLY PATCHED) (function() { 'use strict';

function waitFor(cond, cb, timeout = 8000, interval = 40) { const start = Date.now(); const t = setInterval(() => { try { if (cond()) { clearInterval(t); cb(); } else if (Date.now() - start > timeout) { clearInterval(t); } } catch (e) { clearInterval(t); } }, interval); }

function log(...s) { try { console.log('[SkullPlugin]', ...s); } catch (e) {} }

waitFor(() => (typeof CHARACTERS !== 'undefined' && typeof UNLOCKABLE_PICKUPS !== 'undefined' && typeof ACHIEVEMENTS !== 'undefined' && typeof playerData !== 'undefined' && typeof sprites !== 'undefined' && typeof preRenderEmoji !== 'undefined' && typeof preRenderedEntities !== 'undefined' && typeof draw !== 'undefined' && typeof triggerDash !== 'undefined' && typeof player !== 'undefined' && typeof weaponPool !== 'undefined'), init, 8000);

function init() { log('Initializing skull character plugin...');
...
function createSkullNova() {
  try {
    ...
    const pool = (typeof weaponPool !== 'undefined') ? weaponPool : (window && window.weaponPool);
    const ply  = (typeof player !== 'undefined') ? player : (window && window.player);

    if (!pool || !ply) {
      log('Nova failed: weaponPool or player not found.', !!pool, !!ply);
      return;
    }
    ...
    for (let i = 0; i < NOVA_COUNT; i++) {
      ...
      const weapon = pool.find(w => !w.active);
      if (weapon) {
        weapon.x = ply.x;
        weapon.y = ply.y;
        weapon.size = NOVA_SIZE * (ply.projectileSizeMultiplier || 1);
        weapon.speed = NOVA_SPEED * (ply.projectileSpeedMultiplier || 1);
        weapon.angle = angle;
...
let _lastDash = false;
setInterval(() => {
  const ply = (typeof player !== 'undefined') ? player : window.player;
  if (!ply) return;
  if (ply.isDashing && !_lastDash && ply._isSkull) {
    log('Detected dash via polling — firing nova.');
    createSkullNova();
  }
  _lastDash = !!ply.isDashing;
}, 80);
